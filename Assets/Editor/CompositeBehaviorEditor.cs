...
public class CompositeBehaviorEditor : Editor
{
    ...
    private void AddBehavior(CompositeBehavior _cb)
    {
        int oldCount = (_cb.behaviors != null) ? _cb.behaviors.Length : 0;
        FlockBehavior[] newBehaviors = new FlockBehavior[oldCount + 1];
        float[] newWeights = new float[oldCount + 1];
        for(int i = 0; i < oldCount; i++)
        {
            newBehaviors[i] = _cb.behaviors[i];
            newWeights[i] = _cb.weights[i];
        }
        newWeights[oldCount] = 1f;
        _cb.behaviors = newBehaviors;
        _cb.weights = newWeights;
    }

    private void RemoveBehavior(CompositeBehavior _cb)
    {
        int oldCount = _cb.behaviors.Length;
        if(oldCount == 1)
        {
            _cb.behaviors = null;
            _cb.weights = null;
